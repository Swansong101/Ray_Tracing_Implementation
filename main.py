...
def setup_maze(level):
    for y in range(len(level)):
        for x in range(len(level[y])):
            ...
            elif character == "D":  # Add a door
                doors.append(Door(screen_x, screen_y))  # Create Door instance
...
class Door(turtle.Turtle):
        def __init__(self, x, y):
            turtle.Turtle.__init__(self)
            self.shape("door.gif")  # Adjust the shape name according to your door image
            self.color("brown")  # Adjust the color as needed
            self.penup()
            self.speed(0)
            self.goto(x, y)
...
while True:
    ...
    for enemy in enemies:
        if player.is_collision(enemy):
            print("Player Dies!")
    # Iterate through door list
    for door in doors:
        if player.is_collision(door):
            print("Player found the door!")
            # Add any specific actions for when the player finds the door
    # Update screen
    wn.update()
